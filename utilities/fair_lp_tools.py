# File		: fair_lp_tools.py

"""
This module contains necessary helper functions to build the Cplex lp in alg_if.py
"""
...
def prepare_to_add_constraints(dataset, centres, n_neighbours, P, C, fairness_type = 0):
	...
	Input:
		...
		P, C 	- Flattened variable id for P and C variables defined earlier
	...
	if(fairness_type == 1):
		...
		threshold = distance[selection].reshape(num_samples, n_neighbours)
		
		distance = (distance) / (np.max(threshold, axis = 1)).reshape(-1, 1)
		distance[np.logical_not(selection)] = 1
