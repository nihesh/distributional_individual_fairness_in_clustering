...
"""
This module implements OPT-CF
"""

from kmeans import LoadPickledData, FindClusters
from cplex import Cplex
import numpy as np
from utilities.opt_fair_lp_tools import prepare_to_add_variables
from utilities.stat_lp_tools import prepare_to_add_constraints
from utilities.opt_fair_stat_lp_tools import prepare_to_add_individual_fairness_constraints
...
NUM_NEAREST_NEIGHBOURS = 20							# Number of nearest neighbours to consider for enforcing individual fairness. Note that individual fairness is not ensured if two points don't belong to the same protected group.
...
def opt_fair_stat_clustering(dataset, k, groups, delta, n_neighbours, fairness_type):

	"""
	Implements opt individually and statistically fair clustering algorithm and returns the results of lp solution
	Input:
	...
	rhs, senses, row_names, coefficients = prepare_to_add_constraints(dataset, dataset, P, groups, alpha, beta) 	# Centres is the same as dataset because of kmeans lp
	...
	rhs2, senses2, row_names2, coefficients2 = prepare_to_add_individual_fairness_constraints(dataset, k, groups, P, C, Y, next_eqn_id, n_neighbours, fairness_type)
...
def OptStatisticalIndividualFairness(pickled_dataset, num_clusters, result, fairness_type):

	"""
	Main function for Opt Statistical + Individual fairness algorithm
	"""
	...
	output = opt_fair_stat_clustering(dataset, num_clusters, groups, DELTA, NUM_NEAREST_NEIGHBOURS, fairness_type)
...
if(__name__ == "__main__"):
	...
	target = os.path.join(DUMP_PATH, "opt_statistical_individual_fairness_" + str(NUM_SAMPLES))	
	...
	for num_clusters in NUM_CLUSTERS:				# Compute result for varying cluster size
		for file in files:							# Average over random subsamples of the entire dataset
			...
			result = Result("Opt Statistical + Individual Fairness", num_clusters, file, target)		# Instance of result which will be dumped
			start = time.time()
			OptStatisticalIndividualFairness(data_file, num_clusters, result, FAIRNESS_TYPE)
			end = time.time()
