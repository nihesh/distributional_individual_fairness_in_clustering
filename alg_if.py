...
"""
This module implements ALG-IF
"""

import os
from kmeans import LoadPickledData, FindClusters
from cplex import Cplex
from utilities.fair_lp_tools import *
...
NUM_NEAREST_NEIGHBOURS = 250						# Number of neighbours to enforce fairness constraint (<= NUM_SAMPLES - 1)
QUIET = False										# Doesn't print simulation results to stdout when set to True
FAIRNESS_TYPE = 1
if(len(sys.argv) > 2):
	FAIRNESS_TYPE = int(sys.argv[2])
	
DUMP_PATH = "./Simulation_Dump/fairness_type_" + str(FAIRNESS_TYPE) + "/" + DATASET + "/"	# The folder where results dump gets stored

# Code timing on variable size datasets 
TIMING = False
if(TIMING):
	FAIRNESS_TYPE = 0
	DATASET = "creditcard"
TIMING_NUM_CLUSTERS = 4
TIMING_DATASET_PATH = "./data/processed/" + DATASET + "_variable_size"
TIMING_DUMP_PATH = "./Simulation_Dump/fairness_type_" + str(FAIRNESS_TYPE) + "/" + DATASET + "_variable_size"
...
def PrintSimulationParameters(pickled_dataset, num_clusters, result):
	...
	print("Individual fairness LP implementation\n")
	print(result)


def fair_clustering(dataset, centres, num_nearest_neighbours, fairness_type = 0):

	"""
	Implements fair clustering algorithm and returns the result of lp solution
	Input:
	...
	rhs, senses, row_names, coefficients = prepare_to_add_constraints(dataset, centres, num_nearest_neighbours, P, C, fairness_type)
...
def IndividualFairness(pickled_dataset, num_clusters, result, fairness_type):

	"""
	Main function for Individual Fairness algorithm
	"""

	global QUIET, NUM_SAMPLES, NUM_NEAREST_NEIGHBOURS, TIMING
	...
	output = fair_clustering(dataset, centres, NUM_NEAREST_NEIGHBOURS, fairness_type)
	...
	result.AddCost("Individual fairness", round(fair_cost, 2))
...
if(__name__ == "__main__"):
	...
	if(TIMING):
		...
		files = os.listdir(TIMING_DATASET_PATH)
		files.sort(key=lambda item: (len(item), item))
		target = os.path.join(DUMP_PATH, "variable_size")
		...
		for file in files:
			...
			print("\nSolving " + file + " for num_clusters = " + str(TIMING_NUM_CLUSTERS) + "...")

			data_file = os.path.join(TIMING_DATASET_PATH, file)
			result = Result("Individual Fairness", TIMING_NUM_CLUSTERS, file, target)
			start = time.time()
			IndividualFairness(data_file, TIMING_NUM_CLUSTERS, result, FAIRNESS_TYPE)
			end = time.time()
	...
	target = os.path.join(DUMP_PATH, "individual_fairness_" + str(NUM_SAMPLES))	
	...
	for num_clusters in NUM_CLUSTERS:				# Compute result for varying cluster size
		...
		for file in files:							# Average over random subsamples of the entire dataset
			...
			result = Result("Individual Fairness", num_clusters, file, target)		# Instance of result which will be dumped
			start = time.time()
			IndividualFairness(data_file, num_clusters, result, FAIRNESS_TYPE)
			end = time.time()
